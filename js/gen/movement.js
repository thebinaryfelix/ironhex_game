...
function checkHexCollision(obj1, obj2) {
  ...
  var distanceBetweenCenters = Math.sqrt(Math.pow((obj1.posX - obj2.posX), 2) + Math.pow((obj1.posY - obj2.posY), 2));
...
function randomPosition(game, radius){
  ...
  var position = {
    posX: Math.floor(Math.random() * Math.random() * (game.board.width - 2 * radius) + radius),
